...
{
    public class department_pdf_builder
    {
        Document document;
        string Message;
        string sFilePDF;
        department_model_report _model;
        ...
        public department_pdf_builder(department_model_report model, string FileName, EventHandler<notificationmessageEventArgs> notificationmessageEventname)
        {
            if (model == null)
                throw new ArgumentNullException("department_model_report is null");
            _model = model;

            _notificationmessageEventname = notificationmessageEventname;

            _notificationmessageEventname.Invoke(this, new notificationmessageEventArgs("initialized department_pdf_builder", TAG));

            sFilePDF = FileName;

        }

        public string get_department_pdf()
        {
            try
            {
                BuildPF();
        ...
        private void AddDocHeader()
        {
            ...
            Cell report_title = new Cell(new Phrase("DEPARTMENTS REPORT", hFont1));
        ...
        private void AddDocBody()
        {
            ...
            //Add table details  
            foreach (var d in _model.departments)
            {
                AddTableDetails(d);
            }
        ...
        //table details 
        private void AddTableDetails(print_departments department_model)
        {
            ...
            Cell no = new Cell(new Phrase(department_model.id.ToString(), rms8Bold));
            no.Border = Cell.RECTANGLE;
            no.HorizontalAlignment = Cell.ALIGN_CENTER;
            pdf_table.AddCell(no);

            Cell campus_name = new Cell(new Phrase(department_model.campus_name.ToString(), rms8Bold));
            campus_name.Border = Cell.RECTANGLE;
            campus_name.HorizontalAlignment = Cell.ALIGN_CENTER;
            pdf_table.AddCell(campus_name);

            Cell department_name = new Cell(new Phrase(department_model.department_name.ToString(), rms8Bold));
            department_name.Border = Cell.RECTANGLE;
            department_name.HorizontalAlignment = Cell.ALIGN_CENTER;
            pdf_table.AddCell(department_name);

            Cell status = new Cell(new Phrase(department_model.status, rms8Bold));
            status.Border = Cell.RECTANGLE;
            status.HorizontalAlignment = Cell.ALIGN_CENTER;
            pdf_table.AddCell(status);

            Cell created_date = new Cell(new Phrase(department_model.created_date, rms8Bold));
            created_date.Border = Cell.RECTANGLE;
            created_date.HorizontalAlignment = Cell.ALIGN_CENTER;
            pdf_table.AddCell(created_date);
