...
{
    public class extension_pdf_builder
    {
        Document document;
        string Message;
        string sFilePDF;
        extension_model_report _model;
        ...
        public extension_pdf_builder(extension_model_report model, string FileName, EventHandler<notificationmessageEventArgs> notificationmessageEventname)
        {
            if (model == null)
                throw new ArgumentNullException("extension_model_report is null");
            _model = model;

            _notificationmessageEventname = notificationmessageEventname;

            _notificationmessageEventname.Invoke(this, new notificationmessageEventArgs("initialized extension_pdf_builder", TAG));

            sFilePDF = FileName;

        }

        public string get_extension_pdf()
        {
            try
            {
                BuildPF();
        ...
        private void AddDocHeader()
        {
            ...
            Cell report_title = new Cell(new Phrase("EXTENSIONS REPORT", hFont1));
        ...
        private void AddDocBody()
        {
            ...
            //Add table details  
            foreach (var d in _model.extensions)
            {
                AddTableDetails(d);
            }
        ...
        private void AddTableHeaders()
        {
            ...
            Cell campus_name = new Cell(new Phrase("Campus", tHFont));
            campus_name.Border = Cell.RECTANGLE;
            campus_name.HorizontalAlignment = Cell.ALIGN_CENTER;
            pdf_table.AddCell(campus_name);

            Cell department_name = new Cell(new Phrase("Department", tHFont));
            department_name.Border = Cell.RECTANGLE;
            department_name.HorizontalAlignment = Cell.ALIGN_CENTER;
            pdf_table.AddCell(department_name);

            Cell owner_assigned = new Cell(new Phrase("Owner \nAssigned", tHFont));
            owner_assigned.Border = Cell.RECTANGLE;
            owner_assigned.HorizontalAlignment = Cell.ALIGN_CENTER;
            pdf_table.AddCell(owner_assigned);

            Cell extension_number = new Cell(new Phrase("Extension \nNumber", tHFont));
            extension_number.Border = Cell.RECTANGLE;
            extension_number.HorizontalAlignment = Cell.ALIGN_CENTER;
            pdf_table.AddCell(extension_number);
        ...
        //table details 
        private void AddTableDetails(print_extensions extension_model)
        {
            ...
            Cell no = new Cell(new Phrase(extension_model.id.ToString(), rms8Bold));
            no.Border = Cell.RECTANGLE;
            no.HorizontalAlignment = Cell.ALIGN_CENTER;
            pdf_table.AddCell(no);

            Cell campus_name = new Cell(new Phrase(extension_model.campus_name.ToString(), rms8Bold));
            campus_name.Border = Cell.RECTANGLE;
            campus_name.HorizontalAlignment = Cell.ALIGN_CENTER;
            pdf_table.AddCell(campus_name);

            Cell department_name = new Cell(new Phrase(extension_model.department_name, rms8Bold));
            department_name.Border = Cell.RECTANGLE;
            department_name.HorizontalAlignment = Cell.ALIGN_CENTER;
            pdf_table.AddCell(department_name);

            Cell owner_assigned = new Cell(new Phrase(extension_model.owner_assigned, rms8Bold));
            owner_assigned.Border = Cell.RECTANGLE;
            owner_assigned.HorizontalAlignment = Cell.ALIGN_CENTER;
            pdf_table.AddCell(owner_assigned);

            Cell extension_number = new Cell(new Phrase(extension_model.extension_number, rms8Bold));
            extension_number.Border = Cell.RECTANGLE;
            extension_number.HorizontalAlignment = Cell.ALIGN_CENTER;
            pdf_table.AddCell(extension_number);

            Cell status = new Cell(new Phrase(extension_model.status, rms8Bold));
            status.Border = Cell.RECTANGLE;
            status.HorizontalAlignment = Cell.ALIGN_CENTER;
            pdf_table.AddCell(status);

            Cell created_date = new Cell(new Phrase(extension_model.created_date, rms8Bold));
            created_date.Border = Cell.RECTANGLE;
            created_date.HorizontalAlignment = Cell.ALIGN_CENTER;
            pdf_table.AddCell(created_date);
